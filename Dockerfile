#############################################################################
# raisepartner/grpc-build-wre:1.0.1
#############################################################################
...
RUN wget -q -O cmake-linux.sh https://github.com/Kitware/CMake/releases/download/v$CMAKE_VERSION/cmake-$CMAKE_VERSION-linux-x86_64.sh \
    && /bin/sh cmake-linux.sh -- --skip-license --prefix=$CMAKE_INSTALL_DIR \
    && rm cmake-linux.sh
...
# from official golang/1.17/bullseye/Dockerfile in https://github.com/docker-library/golang
